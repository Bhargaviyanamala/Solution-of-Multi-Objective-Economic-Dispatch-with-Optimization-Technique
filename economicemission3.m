...
function[Fa PgW Pl  PTW  F2]=economicemission(x)
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
       Pl=Pg*B*Pg'+B0i*Pg'+B00;
lam=abs(sum(Pg)+WP-Pd-Pl);
PT=sum(Pg)+lam;
for i=1:n
    F1(i)=(emission(i,1)* Pg(i)^2+emission(i,2)*Pg(i)+emission(i,3)+emission(i,4)*exp(emission(i,5)*Pg(i)));
 F2(i)=data(i,1)* Pg(i)^2+data(i,2)*Pg(i)+data(i,3)+data(i,4)*sin(data(i,5)*(data(i,6)-Pg(i)));
end
       PgW=[x P WP];
PTW=PT+WP;
F2=sum( F2);
